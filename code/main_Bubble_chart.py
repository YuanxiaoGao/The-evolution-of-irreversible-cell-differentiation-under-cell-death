# -*- coding: utf-8 -*-
"""
Created on Thu Oct  3 19:20:42 2024
...
for k in range(10):
    ...
    #给b和c1的每个指数级值都标以序号
        ...
        #print("jin:/n",optimal)
            ...
            def Golbal_initial_ds(M,c,alpha,b,n,SAM):
                ...
                if  np.shape(op_index[0])[0]==1 or n==1:#如果只有一个最优
    ...
    #dp=np.array([[s_ss,s_gs,s_gg];[g_ss,g_gs,g_gg]])
            ...
            with open('qipao_04/%s_%s_%s.txt'%(num_duplicate,dg_num,ds_num),'wb') as f:
